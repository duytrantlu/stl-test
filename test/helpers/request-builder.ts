...
import { ConfigBase } from '../../src/configs/common/config.base';
import { Strategy } from '../../src/configs/common/models';
...
export class RequestBuilder {
  ...
  get(path: string, queryParams?: QueryParams): IRequest {
    return new Request(this.request.get(utils.normalizePath(this.apiUri + path)
      + utils.stringifyQueryParams(queryParams)));
  }

  post(path: string, queryParams?: QueryParams, data?: any): IRequest {
    return new Request(this.request.post(utils.normalizePath(this.apiUri + path)
      + utils.stringifyQueryParams(queryParams)).send(data));
  }

  put(path: string, queryParams?: QueryParams, data?: any): IRequest {
    return new Request(this.request.put(utils.normalizePath(this.apiUri + path)
      + utils.stringifyQueryParams(queryParams)).send(data));
  }

  patch(path: string, queryParams?: QueryParams, data?: any): IRequest {
    return new Request(this.request.patch(utils.normalizePath(this.apiUri + path)
      + utils.stringifyQueryParams(queryParams)).send(data));
  }

  delete(path: string, queryParams?: QueryParams) {
    return new Request(this.request.del(utils.normalizePath(this.apiUri + path)
      + utils.stringifyQueryParams(queryParams)));
  }
}
...
class Response implements IResponse {
  ...
  setValue(fields: { [key: string]: string }[]): this {
    this.response.expect((res: supertest.Response) => {
      const objectPush = fields.reduce((sum: any, field: any) =>
        (Object as any).assign(sum, { [Object.keys(field)[0]]: res.body[field[Object.keys(field)[0]]] })
        , {});
  ...
  endWithSinon(callbackSinon: any): this {
    callbackSinon();
    return this;
  }
}
...
class Request extends Response implements IRequest {
  ...
  userAuth(type: string = ''): this {
    this.auth(StaticValues.TOKEN);
    return this;
  }

  clientSecretAuth(headersSecure: any = { [Strategy.ClientSecret]: ConfigBase.SECURE.API_RESTRICT.CLIENT_SECRET }): this {
    if (headersSecure) {
      Object.keys(headersSecure).forEach(key => {
        this.request.set(key, headersSecure[key]);
      });
    }
    return this;
  }

  attach(field: string, file: MultipartValueSingle,
    options?: string | { filename?: string; contentType?: string }): this {
...
interface IResponse {
  ...
  endWithSinon(callbackSinon: supertest.CallbackHandler): void;
...
interface IRequest extends IResponse {
  ...
  clientSecretAuth(headersSecure?: any): this;
